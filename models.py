...
def init_db(app):
    with app.app_context():
        ...
        pk_def = 'SERIAL PRIMARY KEY' if Config.DB_TYPE == 'postgresql' else 'INTEGER PRIMARY KEY AUTOINCREMENT'
        ...
        if cursor.fetchone()['count'] == 0:
            cursor.execute(
                f'''INSERT INTO users
                   (username, password_hash, full_name, initials, email, role)
                   VALUES ({ph()}, {ph()}, {ph()}, {ph()}, {ph()}, {ph()})''',
                ('admin', generate_password_hash('SuperAdmin@2026'),
                 'System Super Admin', 'SA', 'admin@kemri.go.ke', 'Super Admin')
            )

        conflict = 'ON CONFLICT DO NOTHING' if Config.DB_TYPE == 'postgresql' else 'OR IGNORE'
        for facility in Config.FACILITY_PREFIXES:
            cursor.execute(
                f'INSERT {conflict} INTO id_counters (facility, last_number) VALUES ({ph()}, 0)',
                (facility,)
            )
