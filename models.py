    ...
    with app.app_context():
        ...
        cursor.execute(f"""
            CREATE TABLE IF NOT EXISTS users (
                ...
                role TEXT NOT NULL CHECK (
                    role IN ('Super Admin', 'Data Manager', 'Field Technician')
                )
            )
        """)
        ...
        cursor.execute(f"""
            CREATE TABLE IF NOT EXISTS anc_visits (
                ...
                timestamp TEXT,
                FOREIGN KEY (screening_id)
                    REFERENCES enrolment(screening_id)
                    ON DELETE CASCADE
            )
        """)
        ...
        if cursor.fetchone()["count"] == 0:
            cursor.execute(
                ...
                (
                    "admin",
                    generate_password_hash("SuperAdmin@2026"),
                    "System Super Admin",
                    "SA",
                    "admin@kemri.go.ke",
                    "Super Admin",
        ...
        for facility in Config.FACILITY_PREFIXES:
            if Config.DB_TYPE == "postgresql":
                cursor.execute(
                    f"""
                    INSERT INTO id_counters (facility, last_number)
                    VALUES ({ph()}, 0)
                ...
                cursor.execute(
                    f"""
                    INSERT OR IGNORE INTO id_counters (facility, last_number)
                    VALUES ({ph()}, 0)
                    """,
                    (facility,),
                )
